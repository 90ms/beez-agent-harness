...
const VERIFICATION_FIELDS = new Set(["required", "profiles", "timeoutMs"]);
...
export function validateProjectConfig(project) {
  ...
  if (project.verification !== undefined) {
    ...
    for (const command of requiredCommands) {
      if (!Object.hasOwn(project.commands, command)) {
        throw new Error(
          `Project verification references an unknown command: ${command}`,
        );
      }
    }
    if (project.verification.profiles !== undefined) {
      if (!isPlainObject(project.verification.profiles)) {
        throw new Error("Project verification profiles must be an object");
      }
      const profiles = Object.entries(project.verification.profiles);
      if (profiles.length > 32) {
        throw new Error("Project verification supports at most 32 profiles");
      }
      for (const [name, commands] of profiles) {
        if (!PROFILE_NAME_PATTERN.test(name) || name.length > 64) {
          throw new Error(`Invalid project verification profile name: ${name}`);
        }
        if (
          ...
          throw new Error(
            `Project verification profile must contain command names: ${name}`,
          );
        }
        if (new Set(commands).size !== commands.length) {
          throw new Error(
            `Project verification profile must not contain duplicates: ${name}`,
          );
        }
        for (const command of commands) {
          if (!Object.hasOwn(project.commands, command)) {
            throw new Error(
              `Project verification profile ${name} references an unknown command: ${command}`,
            );
          }
        }
      }
